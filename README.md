...
- запускает локальную OCR-модель TrOCR через PyTorch;
...
Ключевая идея проекта: распознавание выполняется локально, без внешнего OCR API. Веса модели могут быть загружены из Hugging Face при первом запуске, после чего инференс выполняется внутри приложения.
...
Текущая модель `microsoft/trocr-base-handwritten` подходит для проверки архитектуры и локального инференса, но ориентирована в основном на английский рукописный текст. Для русскоязычного дипломного сценария нужно подобрать модель под кириллицу или дообучить OCR-модель.

## Дальнейшее развитие

- собрать тестовый набор изображений с эталонными расшифровками;
- подобрать или дообучить модель под русский рукописный текст;
